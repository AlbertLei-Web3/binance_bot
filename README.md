...
- **信号引擎** — 8 个做空信号检测器 + 加权评分系统
- **引爆信号检测** — 5m/15m 双周期确认，捕捉冲高回落与放量暴跌短周期异动
...
│   ├── screener.py                 # 标的自动筛选
│   ├── signal_engine.py            # 信号引擎（8 个检测器）
│   └── ignition_detector.py        # 引爆信号检测器（5m/15m 短周期）
├── utils/                          # 工具模块
│   ├── indicators.py               # 技术指标库
│   └── logger.py                   # 日志
├── scripts/
...
| MACD 死叉 | 1.3 | 柱状图转负 |
| 量价背离 | 1.3 | 上涨缩量或下跌放量 |
| **引爆信号** | **1.4** | **5m/15m 双周期确认：冲高回落或放量暴跌** |
| 价格回落 | 1.2 | 从高点回落 5%+ |
...
当 1h 信号 ≥ 2 个时，自动拉取 5m/15m K 线做短周期异动检测：

- **冲高回落** — 长上影线（≥ 实体 1.5 倍）+ 冲高放量 + 后续缩量
- **放量暴跌** — 单根大阴线（跌幅达标 + 实体占比 ≥ 60% + 放量 ≥ 2x）或连续阴线累计跌幅达标

同一形态需在 5m 和 15m 双周期同时出现才确认触发。可通过 `require_ignition_for_entry` 配置为入场必要条件。
...
altcoin_short:
  ...
  require_ignition_for_entry: false  # 是否要求引爆信号才入场
...
ignition:                      # 引爆信号阈值（5m 更灵敏，15m 更宽松）
  weight: 1.4
  5m:
    spike_surge_pct: 0.015
