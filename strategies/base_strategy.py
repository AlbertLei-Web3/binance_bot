...
class BaseStrategy(ABC):
    ...
    @staticmethod
    def get_klines_multi_timeframe(symbol: str,
                                   timeframes: Dict[str, int] = None) -> Dict[str, list]:
        """
        获取多个时间框架的 K 线数据

        Args:
            symbol: 交易对
            timeframes: {"interval": limit} 例如 {"1h": 100, "4h": 50}
        ...
        for interval, limit in timeframes.items():
            result[interval] = get_klines(symbol, interval=interval, limit=limit)
        return result
